    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Review invoice status, due amounts, and payment outcomes.</p>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                                ...
                                <th class="px-6 py-3">Amounts</th>
                                <th class="px-6 py-3">Issued / Due</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @forelse($invoices as $invoice)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold {{ $invoice->failed_at ? 'bg-red-50 text-red-700' : ($invoice->status === 'paid' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-700') }}">
                                            {{ $invoice->failed_at ? 'Payment failed' : ucfirst(str_replace('_', ' ', $invoice->status)) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        <div>Total: {{ strtoupper($invoice->currency) }} {{ number_format($invoice->total / 100, 2) }}</div>
                                        <div>Paid: {{ strtoupper($invoice->currency) }} {{ number_format($invoice->amount_paid / 100, 2) }}</div>
                                        <div>Due: {{ strtoupper($invoice->currency) }} {{ number_format($invoice->amount_due / 100, 2) }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        <div>{{ $invoice->issued_at?->format('M d, Y') ?? 'N/A' }}</div>
                                        <div>{{ $invoice->due_at?->format('M d, Y') ?? ($invoice->paid_at?->format('M d, Y') ?? 'No due date') }}</div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
